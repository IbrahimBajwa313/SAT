...
export default function ReliefActivitySummary({ activity, image, progress = 75, goal = 5000, initialStatus = 'active' }) {
  ...
  const statusText = widgetState === 'urgent' ? 'Urgent'
    : widgetState === 'goalReached' ? 'Goal Reached'
    : widgetState === 'closed' ? 'Closed'
    : 'Active';
  const tooltipText = `Helped ${Math.floor(currentProgress * 4)} families so far.`;

  return (
    <div
      className="relative bg-gradient-to-br from-white to-gray-50 rounded-2xl p-6 transform transition-all duration-300 hover:shadow-xl hover:-translate-y-2"
    >
      {/* Activity Image */}
      <div className="mb-4">
        <img
      ...
      {/* Activity Title */}
      <h4 className="text-lg font-medium text-gray-800 text-center mb-3">{activity}</h4>
